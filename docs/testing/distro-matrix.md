...
A4Diag 0.4.3 通过 `.github/workflows/test.yml` 和 `release.yml` 在所有受支持发行版上执行 CI 验证：
...
- **build**：构建精确的 `a4diag-0.4.3-py3-none-any.whl` 和 `a4diag_builtin_plugins-0.4.3-py3-none-any.whl`；运行 `verify-source` 检查固定目标字面量，并运行 `verify-release`。
