...
本文描述 `feat/linux-fault-recovery` 开发分支，**尚未发布，不包含在 v0.5.1 中**。使用本功能需要从同一开发版本构建并部署控制端、内置插件和目标端；仅向已发布版本加入这些配置不会启用功能。部署流程沿用 [安装指南](install.md)，服务与 HTTP 检查见 [服务恢复说明](deployment/service-http-recovery.md)。
...
目标 RPC 只传 `probe_id`，不传探针定义。目标端按自己的策略解析 ID。因此，不能仅核对 ID 相同：例如两端将同一个文件探针 ID 指向不同文件，控制端的恢复检查就可能检查了错误对象。修改定义时应同步两端，并重新采集确认。省略 `diagnostic_probes` 等价于空列表，不存在隐式默认探针。
