...
本功能从 v0.5.0 开始提供；v0.5.1 修复真实模型通信及 SSH 身份指纹问题。部署请使用同为 0.5.1 的控制端、内置插件和目标端构建产物，不能只替换配置文件。升级顺序见 [v0.5.1 发布说明](../release/v0.5.1.md)。
...
### 复杂问题的模型预算（开发分支，尚未包含在 v0.5.1）

初始化请求的 `model.max_tokens` 默认 8192，可配置为 64–16384；`model.timeout_seconds` 默认 120 秒，可配置为 1–300 秒。例如复杂诊断可设置 `"max_tokens": 16384, "timeout_seconds": 180`。诊断、计划和审查各自使用这个单次调用上限，初始化探测也继承模型配置。插件宿主和控制端 RPC 等待时间会随模型超时调整，服务操作自身的执行超时保持独立。

这里设置的是提供方 API 的 `max_tokens` 输出预算，不是总上下文长度或独立的推理强度选项；具体模型可能将推理 token 计入该预算。提高上限可能增加调用时间和费用，不能替代证据质量和恢复验证。

已有配置中的显式值不会被默认值覆盖。修改原始初始化 JSON 后重新运行 `a4diag init --input`，使核心配置与插件配置同时更新。复杂问题仍最多补采两轮已登记证据；证据不足、输出截断、计划审查不完整会停止写入，高风险操作仍需审批。
