...
class ReleaseBuildContractTests(unittest.TestCase):
    ...
    def create_assembly_fixture(
        ...
        core_wheel = root / "a4diag-0.5.0-py3-none-any.whl"
        builtin_wheel = root / "a4diag_builtin_plugins-0.5.0-py3-none-any.whl"
        ...
        self.write_minimal_wheel(core_wheel, "a4diag", "0.5.0")
        self.write_minimal_wheel(builtin_wheel, "a4diag-builtin-plugins", "0.5.0")
        return (
            project,
    ...
    def test_assembled_release_is_complete_and_hash_manifested(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                (output / "VERSION").read_text("utf-8").strip(), "0.5.0"
            )
    ...
    def test_target_release_is_exact_and_independently_verifiable(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for filename, distribution in (
                ("a4diag-0.5.0-py3-none-any.whl", "a4diag"),
                ("a4diag_builtin_plugins-0.5.0-py3-none-any.whl", "a4diag-builtin-plugins"),
                ("a4diag_target_runtime-0.5.0-py3-none-any.whl", "a4diag-target-runtime"),
            ):
                wheel = root / filename
                self.write_minimal_wheel(wheel, distribution, "0.5.0")
                wheels.append(wheel)
    ...
    def build_v3_wheel(self, wheel_dir: Path) -> Path:
        ...
        self.assertEqual([wheel.name for wheel in wheels], ["a4diag-0.5.0-py3-none-any.whl"])
        return wheels[0]
    ...
    def test_installed_wheel_reports_v3_package_version(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(observed["version"], "0.5.0")
